...
class TrainerMT(MultiprocessingEventLoop):
    ...
    def __init__(self, encoder, decoder, latent, latent_joint, discriminator, lm, data, params):
        ...
        self.model_opt = {
            ...
            'lat': (self.latent, self.lat_optimizer),
            'lat_joint': (self.latent_joint, self.lat_joint_optimizer),
        ...
        for lang in params.mono_directions:
            ...
            self.stats['kld_%s_%s' % (lang, lang)] = []
    ...
    def enc_dec_step(self, lang1, lang2, lambda_xe, back=False):
        ...
        mu_lat, var_lat = self.latent(encoded.enc_hiddens, encoded.input_len, lang1_id)
        ...
        if back:
            ...
            self.stats['kld_bt_%s_%s' % (lang1, lang2)].append(kld.item())
        else:
            self.stats['xe_costs_%s_%s' % (lang1, lang2)].append(xe_loss.item())
            self.stats['kld_%s_%s' % (lang1, lang2)].append(kld.item())
    ...
    def otf_bt(self, batch, lambda_xe, backprop_temperature):
        ...
        if backprop_temperature == -1:
            ...
            mu_lat, var_lat = self.latent(encoded.enc_hiddens, encoded.input_len, lang2_id)
            latent_resampled = self.latent.reparameterize(mu_lat, var_lat)
        else:
            # lang1 -> lang2
            encoded = self.encoder(sent1, len1, lang_id=lang1_id)
            ...
            mu_lat, var_lat = self.latent(encoded.enc_hiddens, encode.input_len, lang2_id)
            latent_resampled = self.latent.reparameterize(mu_lat, var_lat)
