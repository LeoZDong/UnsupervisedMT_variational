...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        lstm = [
            nn.LSTM(self.emb_dim + self., self.hidden_dim, num_layers=self.n_enc_layers, dropout=self.dropout)
    ...
    def forward(self, x, lengths, lang_id):
        ...
        return LatentState(input_len=lengths, dec_input=latent_state, dis_input=latent_state, enc_hiddens=lstm_output)
...
class Decoder(nn.Module):
    ...
    def forward(self, encoded, latent_resampled, y, lang_id, one_hot=False):
        """
        Input:
            - LongTensor of size (slen, bs), word indices
              or
            ...
            - FloatTensor of size (bs, latent_dim), variational latent space
              representing the sentence (conditioned on encoder language)
        Output:
        ...
        latent = encoded.dec_input # NOTE: THIS IS DIFFERENT FROM THE VARIATIONAL SPACE!!
        ...
        if self.init_encoded:
            ...
            init = None
            # copy encoded and latent_resampled slen times because they are the same for each time step
            encoded = latent.unsqueeze(0).expand(slen, bs, self.enc_dim)
            latent_resampled_reshaped = latent_resampled.unsqueeze(0).expand(slen, bs, self.latent_dim)
            lstm_input = torch.cat([embeddings, encoded, latent_resampled_reshaped], 2)
        ...
        return scores.view(slen, bs, n_words), lstm_output
    ...
    def generate(self, encoded, latent_resampled, lang_id, max_len=200, sample=False, temperature=None):
        ...
        """
        assert encoded.input_len.size(0) == encoded.dec_input.size(0)
        latent = encoded.dec_input # NOTE: DIFFERENT FROM latent_resampled!
        ...
        while cur_len < max_len:
            ...
            # concatenate resampled latent space to input of the decoder
            embeddings = torch.cat((embeddings, latent_resampled), 1)
...
class Latent(nn.Module):
    def __init__(self, params):
        """
        Latent space initialization. Conditioned on one language.
        Corresponds to q(z|s) or q(z|t) in paper.
        Input:
            - params object
            - FloatTensor of size (seq_len, bs, hidden_dim) representing all hidden
            states of each sentence, for all sentences in a batch.
        """
    ...
    def reparameterize(self, mu, logvar):
        ...
        Input:
            - FloatTensor of size (bs, latent_dim) representing mean of latent space,
            treating each dimension independently as one variable.
            - FloatTensor of size (bs, latent_dim) representing log variance of latent
            space, treating each dimension independently as one variable.
        Output:
            - FloatTensor of size (bs, latent_dim). In training mode, output sampled
            latent space representation with log variance as noise. In evaluation mode,
            output mean of latent space directly.
        """
        if self.training:
            ...
            # During inference, simply return the mean of the learned distribution of the
    ...
    def forward(self, enc_hiddens, lang_id):
        ...
        var = var_layer(enc_hiddens_mean) # (bs, latent_dim)
        return mu, var


class LatentJoint(nn.Module):
    def __init__(self, params):
        """
        Latent space initialization. Conditioned on both languages. Since it is
        shared between forward and backward networks, make sure the source_hiddens
        and target_hiddens are passed in in correct order.
        Corresponds to q(z|s,t) in paper.
        Input:
            - params object
            - FloatTensor of size (seq_len, bs, hidden_dim) representing all hidden
            states of each sentence in the source language, for all sentences in a batch.
            - FloatTensor of size (seq_len, bs, hidden_dim) representing all hidden
        ...
        self.var = nn.Linear(self.hidden_dim * 2, latent_dim)
...
def build_seq2seq_model(params, data, cuda=True):
    ...
    logger.info("")
    logger.info("============ Building seq2seq model - Latent")
    latent = Latent(params)
    logger.info("")
    logger.info("============ Building seq2seq model - Latent Joint")
    latent_joint = LatentJoint(params)
    logger.info("")
