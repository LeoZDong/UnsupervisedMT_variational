...
class Decoder(nn.Module):
    ...
    def __init__(self, params, encoder):
        ...
        # LSTM layers / shared layers
        self.lstm1_input_size = self.emb_dim + (self.emb_dim if self.input_feeding else 0) + self.latent_dim
    ...
    def forward(self, encoded, latent_resampled, y, lang_id, one_hot=False):
        ...
        if self.input_feeding:
            ...
            for i in range(y_len):
                ...
                # logger.info("latent resampled reshaped size:{}".format(latent_resampled_reshaped.size()))
                lstm_input = torch.cat([lstm_input, attention, latent_resampled_reshaped], 2)
                lstm_layer1.flatten_parameters()
            ...
            if self.n_dec_layers > 1:
                ...
                lstm_layer2.flatten_parameters()
            ...
            # first LSTM layer
            latent_resampled_reshaped = latent_resampled.unsqueeze(0).expand(y_len, bs, self.latent_dim)
            lstm_input = torch.cat([embeddings, latent_resampled_reshaped], 2)
            lstm_layer1.flatten_parameters()
            lstm_output, (_, _) = lstm_layer1(lstm_input)
            assert lstm_output.size() == (y_len, bs, self.hidden_dim)
    ...
    def generate(self, encoded, latent_resampled, lang_id, max_len=200, sample=False, temperature=None):
        ...
        while cur_len < max_len:
            ...
            # lstm step
            lstm_input = embeddings.unsqueeze(0) # (1, bs, self.emb_dim)
            if self.input_feeding:
                lstm_input = torch.cat([lstm_input, attention], 2)
            latent_resampled_reshaped = latent_resampled.unsqueeze(0) # (1, bs, self.latent_dim)
            lstm_input = torch.cat((lstm_input, latent_resampled_reshaped), 2)
    ...
    def generate_beam(self, encoded, latent_resampled, lang_id, beam_size=20, max_len=175, sample=False, temperature=None):
        """
        Generate a sentence from a given initial state.
        Input:
            - FloatTensor of size (batch_size, hidden_dim) representing
...
class Latent(nn.Module):
    ...
    def forward(self, enc_hiddens, input_len, lang_id):
        # (bs, hidden_dim)
        enc_hiddens_mean = torch.sum(enc_hiddens, dim=0) / torch.tensor(input_len, dtype=torch.float)[:, None]
...
def build_attention_model(params, data, cuda=True):
    ...
    logger.info("Latent: {}".format(latent))
    logger.info("LatentJoint: {}".format(latent_joint))
    logger.info("")
