...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        # variational layers
        self.z_mu = nn.Linear(self.emb_dim, self.embd_dim)
        self.z_var = nn.Linear(self.embd_dim, self.embd_dim)
    ...
    def reparameterize(self, mu, logvar):
        """ *New as part of variational NMT*
        ...
        if self.training:
            ...
            std = torch.exp(0.5*logvar)  # type: Variable
            # std = torch.sigmoid(logvar)
            # in the same size as std
